...
# Load data
sc_features, sc_adj, fc_adj, fc_features = loadFSData("G:\Data\data_fMRI_DTI\\xinxiang_data_fmri_dti\HC", "G:\Data\data_fMRI_DTI\\xinxiang_data_fmri_dti\HC")
...
def test_epoch(test_sess, epochs):
    ...
    for test_sub in index:
        ...
        for test_i in test_test_it:
            ...
            value_outs = test_sess.run([lstmGAN.pred_feature_sub, lstmGAN.label_feature_sub], feed_dict=test_feed_dict)
    ...
    print("Test cost mse: " + str(test_cost / len(index)))
    print("Test cost feature_mse: " + str(test_acc / len(index)))
    print("Test cost total_Pre_loss: " + str(test_rec / len(index)))
    print("Test cost precision:" + str(test_pre / len(index)))
    f = open('test_result_0_0.5.txt', 'a')
    f.write('GAN_gcn_lstm_pre_train_' + str(FLAGS.pre_epochs) + '_train_' + str(epochs) + ' Test cost mse: ' + str(test_cost / len(index))
            + ' Test cost accuracy: ' + str(test_acc / len(index)) + ' Test cost recall: ' + str(test_rec / len(index))
            + ' Test cost precision: ' + str(test_pre / len(index)) + '\n')
    f.close()
    hm_graph = hm_graph / len(index)
...
for epoch in range(FLAGS.pre_epochs):
    ...
    ind = list(range(len(test_adj_sc)))
    random.shuffle(ind)
    for i in ind:
        # Construct feed dictionary
        feed_dict = construct_feed_dict(test_adj_sc[i], test_adj_fc[i], test_adj_label[i], test_features_sc[i], test_features_fc[i],
                                        test_adj_fc_pre[i], test_labels_feature[i], placeholders)
        ...
        outs = sess.run(
            [lstmGAN.mse, lstmGAN.mse, lstmGAN.feature_mse, lstmGAN.mae, lstmGAN.feature_mae,
    ...
    avg_pre_loss = avg_pre_loss/len(test_adj_sc)
    avg_pre_acc = avg_pre_acc/len(test_adj_sc)
    avg_pre_rec = avg_pre_rec/len(test_adj_sc)
    avg_pre_pre = avg_pre_pre/len(test_adj_sc)
    ...
    if epoch % 10 == 0:
        saver.save(sess, 'model/HA_dense_SC_pre_much_%r' % epoch, global_step=epoch)
print("Pre Training Finished!")
test_epoch(sess, FLAGS.pre_epochs)
#
# # Train model
...
#     avg_d_loss = avg_d_loss/len(index)
#     avg_g_loss = avg_g_loss/len(index)
#     avg_mse_loss = avg_mse_loss/len(index)
